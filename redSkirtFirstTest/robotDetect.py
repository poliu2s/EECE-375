...
class robotDetect:
    ...
    def findRobot(self):
        ...
        image = cv.LoadImageM('bigblue_manualwarp.jpg')


        #Using RGB to filter for red
        redFilter = cv.CreateImage(cv.GetSize(image), 8, 1)
        cv.InRangeS(image, cv.Scalar(0,0,170), cv.Scalar(100,100,255), redFilter)   #Filter for red - BGR
        cv.SaveImage('bigblue_manualwarpRedFilter.jpg', redFilter)
        ...
        cv.SaveImage('bigblue_manualwarpRedFilter1.jpg', debugPic1)
        ...
        cv.SaveImage('bigblue_manualwarpRedFilter2.jpg', debugPic2)
