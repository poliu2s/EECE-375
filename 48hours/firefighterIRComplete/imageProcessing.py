...
class ecePredator:
    ...
    def findWhiteCandles(self, corners):
        ...
        #cv.InRangeS(self.image, cv.Scalar(110,110,110), cv.Scalar(255,255,255), imgThresholdW)
        #cv.InRangeS(self.image, cv.Scalar(100,100,100), cv.Scalar(255,255,255), imgThresholdW)

        cv.InRangeS(self.image, cv.Scalar(140,140,140), cv.Scalar(255,255,255), imgThresholdW)
        #--------------------------------------------------------
        ...
        destiny = cv.CreateImage(cv.GetSize(self.image),8,1)
        for i in range(0, len(volatile1)):
                destiny[volatile1[i][0],volatile1[i][1]] = 255
        cv.SaveImage('finalfieldFindingCandlelightWhite2volatile1.jpg', destiny)



        THRESHOLD = 30 ###The variable to alter; hasworked: 50-200
        BOT_THRESH = 2
        self.whiteBottoms = []
        ...
        finalWhiteBaseList = []
        for i in range(50, 550):            ##Anuj
                for j in range(50, 550):     ##Anuj
                        if (warpWgray[i, j] > 0):
    ...
    def findBlackCandles(self, corners2):
        ...
        cv.SaveImage('finalfieldedgePointsB.jpg', destiny)
...
##        print '----------------'
##        print self.blackBottoms
##        print '-----------------'
##        print len(self.blackBottoms)
        ...
        finalBlackBaseList = []
        for i in range(60, 540):         ##Anuj
                for j in range(60, 540): ##Anuj
                        if (warpBgray[i, j] > 0):
        ...
        print len(finalBlackBaseList)
        print finalBlackBaseList
