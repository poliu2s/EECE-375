...
class eceCorners:
    ...
    def findCorners(self, picture_name):
        ...
        ###Calibration!!!!!
        #cv.InRangeS(imgHSV, cv.Scalar(100,40,160), cv.Scalar(130,240,300), imgThreshold)
        #cv.InRangeS(imgHSV, cv.Scalar(100,40,160), cv.Scalar(130,210,300), imgThreshold)
        cv.InRangeS(imgHSV, cv.Scalar(100,40,170), cv.Scalar(130,210,300), imgThreshold)
        ...
        if USE_STANDARD:
                ...
                #####################################################
                ##Be calm and calibrate!!!
                ###########################
                #lines = cv.HoughLines2(dst, storage, cv.CV_HOUGH_PROBABILISTIC, 1, pi / 180, 50, 50, 90)
                lines = cv.HoughLines2(dst, storage, cv.CV_HOUGH_PROBABILISTIC, 1, pi / 180, 50, 50, 200)


                # Po's code

                #Find the vertical lines
                verticalDistanceTest = 170
                linesV = [ 0 ]
                linesV.pop(0)
                for line in lines:
